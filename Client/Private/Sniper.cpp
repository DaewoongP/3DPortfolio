...
HRESULT CSniper::Initialize_ParentMatrix(PARENTMATRIXDESC ParentDesc)
{
...
void CSniper::Tick(_double dTimeDelta)
{
...
GAMEEVENT CSniper::Late_Tick(_double dTimeDelta)
{
...
HRESULT CSniper::Add_Components()
{
	/* For.Com_Renderer */
	if (FAILED(__super::Add_Component(LEVEL_STATIC, TEXT("Prototype_Component_Renderer"),
		TEXT("Com_Renderer"), (CComponent**)&m_pRendererCom)))
	{
		MSG_BOX("Failed CSniper Add_Component : (Com_Renderer)");
		return E_FAIL;
	}

	/* For.Com_Model */
	if (FAILED(__super::Add_Component(LEVEL_STAGE2, TEXT("Prototype_Component_Model_Weapon_Sniper"),
		TEXT("Com_Model"), (CComponent**)&m_pModelCom)))
	{
		MSG_BOX("Failed CSniper Add_Component : (Com_Model)");
		return E_FAIL;
	}
	...
	{
		MSG_BOX("Failed CSniper Add_Component : (Com_Shader)");
		return E_FAIL;
	}
...
void CSniper::Attack(_vector vPosition, _vector vTargetPos, _double dSpeed)
{
	CGameInstance* pGameInstance = CGameInstance::GetInstance();
	Safe_AddRef(pGameInstance);
	...
	TransformDesc.dRotationPerSec = 0.0;
	TransformDesc.dSpeedPerSec = dSpeed;
	pGameObject->Get_Transform()->Set_Desc(TransformDesc);
	pGameObject->Get_Transform()->Set_Scale(_float3(5.f, 5.f, 5.f));
...
void CSniper::Bullet_Late_Tick(_double dTimeDelta)
{
