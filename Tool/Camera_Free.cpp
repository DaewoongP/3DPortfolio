...
void CCamera_Free::Mouse_Move(_double dTimeDelta)
{
	...
	{
		_vector	vRight = m_pCamera->Get_TransformState(CTransform::STATE::STATE_RIGHT);
		
		m_pCamera->Turn(vRight, (dwMouseMove / 10.f) * dTimeDelta);
	}
	...
	if (dwMouseMove = pGameInstance->Get_DIMouseMove(DIMS_X))
	{
		...
		m_pCamera->Turn(vUp, (dwMouseMove / 10.f) * dTimeDelta);
	}
}
